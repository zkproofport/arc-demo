...
(() => {
  ...
  const steps = [
    {id:"mandate",nav:"Mandate",duration:6000,kicker:"01 / THE AGENT'S MISSION",title:"A specific action. A bounded budget.",location:"OFFCHAIN · AGENT STACK",caption:"User authorization comes first. The agent checks the service policy, quote and available funds.",decision:"Request eligibility",reason:"The vault requires a valid Coinbase KYC proof for Wallet B."},
    ...
    {id:"receipt",nav:"Receipt",duration:5000,kicker:"07 / THE RESULT",title:"An eligible action. A reviewable receipt.",location:"ARC OUTCOME · SIMULATED",caption:"The service sees eligibility and the authorized action. This run has no live proof, deployed gate or transaction hash.",decision:"Mission complete",reason:"Simulated deposit succeeded. The proof fee is still pending Gateway batch settlement."}
  ];
  ...
  class ProtocolRenderer{
    ...
    render(frame){const index=steps.findIndex(s=>s.id===frame.stepId);if(index<0)throw new Error("Unknown phase");const step=steps[index],p=frame.progress,scenario=frame.scenario||"valid",stop=scenarios[scenario].stop;const blocked=stop!==undefined&&(index>stop||(index===stop&&p>(scenario==="budget"?.42:.85)));
      ...
      this.text("map-verdict",blocked?scenarios[scenario].code:index===6?"10 USDC deposited · nonce consumed":"Verify → consume nonce → deposit");
